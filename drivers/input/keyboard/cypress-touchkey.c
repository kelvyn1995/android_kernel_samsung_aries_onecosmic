...
bool bln_enabled = false;		// indicates if BLN function is enabled/allowed
...
static void enable_touchkey_backlights(void){
	i2c_touchkey_write_byte(blndevdata, OLD_BACKLIGHT_ON);
}

static void disable_touchkey_backlights(void){
	i2c_touchkey_write_byte(blndevdata, OLD_BACKLIGHT_OFF);
}
		...
		const struct i2c_device_id *id)
{
	...
	if (err) {
		...
		/*
		 *  this is needed to get access to the platformdata,
		 *  misc driver does not give access to it
		 *  (or i did miss that somewhere)
		 */
		blndevdata = devdata;
